...
process_exp_data_hist_fut <- function(experiment, year_k){
...
#  fair_exps <- listofdfs[[2]]


  # ... and append all dataframes
  fair_exps <- do.call(rbind, listofdfs)
  #head(fair_exps)
  #fair_exps <- listofdfs[[1]]
  #unique(fair_exps$experiment_iso)
  
#  fair_exps <- fair_exps %>% 
 #   dplyr::group_by(year, experiment_iso) %>% 
  #  dplyr::summarise(resp = median(as.numeric(Test)))
  
#  fair_exps1 <- subset(fair_exps, experiment_iso == "2099")
#  plot(fair_exps$year, fair_exps$resp)
#  plot(fair_exps1$year, fair_exps1$resp)
  ...
  fair_exps <- fair_exps %>% 
    dplyr::mutate(loop = case_when(num_loop <= 9 ~ paste0("00", loop),
                                   ...
                                   TRUE ~ loop))
#head(fair_exps)  
#  fe_1990a <- subset(fair_exps, experiment_iso == 1990)
  
  #  fe_1990a <- fair_exps %>% dplyr::group_by(year) %>% 
  # dplyr::summarise(median = median(as.numeric(Test), na.rm = T))
  
  #  plot(fe_1990a$year, fe_1990a$median, type = "l")
  #plot(fe_1990$year, fe_1990$median, type = "l", col ="red")
  
  #  fes <- left_join(fe_1990a, fe_1990,
  #                 by = c("year"))
  
  #  fes$diff <- fes$median.x - fes$median.y
