...
calculate_damages_pulse_lags <- function(ratio_raster, experiment_df, list_of_exps, 
                                         year_k, future_forecast, gdp_temp_dataset, 
                                         temp_dataset, settlement_year, growth_past_2100, 
                                         adaptation, bootstrapped, bhm_lag_model){
  ...
  for (i in list_of_exps){
    ...
    # read the bhm model under lags 
    bhm_model  <- bhm_lag_model
    
    #assign teh coeffeceints to the country-year dataset
    gdp_temp_data1$temp <- bhm_model$temp
    gdp_temp_data1$temp2 <- bhm_model$temp2
    ...
    #gdp_temp_data1$temp_l6 <- coef(bhm_model)[13]
    #gdp_temp_data1$temp2_l6 <- coef(bhm_model)[14]
    #gdp_temp_data1$temp_l7 <- coef(bhm_model)[15]
    #gdp_temp_data1$temp2_l7 <- coef(bhm_model)[16]
    #gdp_temp_data1$temp_l8 <- coef(bhm_model)[17]
    #gdp_temp_data1$temp2_l8 <- coef(bhm_model)[18]
    #gdp_temp_data1$temp_l9 <- coef(bhm_model)[19]
    #gdp_temp_data1$temp2_l9 <- coef(bhm_model)[20]
    #gdp_temp_data1$temp_l10 <- coef(bhm_model)[21]
    #gdp_temp_data1$temp2_l10 <- coef(bhm_model)[22]
