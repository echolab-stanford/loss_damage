...
calculate_bidamages <- function(ratio_raster, experiment_df, list_of_exps, 
                                year_k, future_forecast, gdp_temp_dataset, experiment_year){
  ...
  #  sum(damages_i_t4$weighted_damages2_scld[damages_i_t4$weighted_damages2_scld > 0], na.rm = T)
    

    #damages_i_t4 <- subset(damages_i_t4, weighted_damages2_scld < 0 )
    
    #  negats <- damages_i_t4 %>% subset(., weighted_damages2_scld < 0 & year <= 1987) %>% 
    #  dplyr::group_by(emitter, year) %>% 
    #  dplyr::summarise(total = sum(weighted_damages2_scld, na.rm = T),
    #                   .groups = "keep")
    
    #  posats <- damages_i_t4 %>% subset(., ISO3 == "DEU", weighted_damages2_scld > 0 & year <= 1987) %>% 
    #  dplyr::group_by(emitter, year) %>% 
    #  dplyr::summarise(total = sum(weighted_damages2_scld, na.rm = T),
    #                   .groups = "keep")
    
    #  deu <- damages_i_t4 %>% subset(., ISO3 == "DEU" & year <= 1987) %>% 
    #dplyr::group_by(emitter, year) %>% 
    # dplyr::summarise(total = sum(weighted_damages2_scld, na.rm = T),
    #                  .groups = "keep")
    
    
    #  plot(deu$year, deu$total, type = "l")
    
    #  lines(posats$year, posats$total, type = "l", col = "red")
    
    #  change <- damages_i_t4 %>% subset(., year <= 1987) %>% 
    #  dplyr::group_by(emitter, ,ISO3 ,year) %>% 
    #  dplyr::summarise(total = sum(weighted_damages2_scld, na.rm = T),
    #                   .groups = "keep")
    #  change1 <- change %>% subset(., year == 1983)
    #  change2 <- change %>% subset(., year == 1984)
    
    # change_all <- left_join(change1, change2, by = c("ISO3"))
    
    # change_all$change <- abs(change_all$total.y - change_all$total.x)
    
    #  totds <- damages_i_t4 %>% dplyr::group_by(emitter, year, ISO3) %>% 
    #    dplyr::summarise(totals = sum(weighted_damages2_scld, na.rm = T))
    
    #totds <- totds %>% subset(.,year == 1985 | year == 1984)
    
    #  sum(totds$totals, na.rm = T)
...
calculate_bidamages <- function(ratio_raster, experiment_df, list_of_exps, 
 ...
 # i <- "ExxonMobil"
  ...
  #year_k <- 1990  
    ...
    gdp_temp_data1 <- subset(gdp_temp_data1, year >= 1990)
