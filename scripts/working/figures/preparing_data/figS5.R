...
damages_per_1gtco2yr <- readRDS(paste0(output_path, "20230628/total_damages_1gtco2_1980_2020.rds"))

#damages_per_1gtco2yr <- total_damages_1tCO2
# now let calculate total carbon deby by year and emitter
damages_per_1gtco2yr_sum <- damages_per_1gtco2yr %>% 
  dplyr::group_by(emitter, year) %>% 
  dplyr::summarise(total_damages = sum(weighted_damages2_scld, na.rm = T))
colnames(damages_per_1gtco2yr_sum)[1] <- "emission_year"
...
carb_majors_jet <- left_join(carb_majors_jet, damages_per_1gtco2yr_sum,
...
celebsjet <- left_join(celebsjet, damages_per_1gtco2yr_sum,
                       by = c("years" = "emission_year"))
...
elonjet <- left_join(elonjet, damages_per_1gtco2yr_sum,
                     by = c("years" = "emission_year"))
...
bezosjet <- left_join(bezosjet, damages_per_1gtco2yr_sum,
                      by = c("years" = "emission_year"))
...
corpsjet <- left_join(corpsjet, damages_per_1gtco2yr_sum,
                      by = c("years" = "emission_year"))
...
sportsjet <- left_join(sportsjet, damages_per_1gtco2yr_sum,
                       by = c("years" = "emission_year"))
...
gatesjet <- left_join(gatesjet, damages_per_1gtco2yr_sum,
                      by = c("years" = "emission_year"))
...
# let us do private jets 
celebsjet <- subset(celebsjet, years == 2022) 
mburkedf <- left_join(mburkedf, damages_per_1gtco2yr_sum,
                      by = c("years" = "emission_year"))
...
all_celebs_tot <- all_celebs_tot %>% 
  dplyr::mutate(emitter = case_when(emitter == "Marshall Burke's Flights" ~ paste0(emitter, " ($1.1)"),
                                    TRUE ~ emitter))
