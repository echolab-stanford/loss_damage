...
# This R script is to produce the sankey diagrams
# this script prep data for figurs 4, and S4,5,6,7
...
#############################################################################
# read data 
total_damages_k90 <- readRDS(paste0(output_path, "/total_damages_k90_v2022.rds"))
...
prep_data_for_sankey <- function(dataset){
  ...
  damages_strictly <- dataset %>% dplyr::group_by(emitter, reciever) %>% 
    dplyr::summarise(transfer = sum(weighted_damages2[status_transfer == 'damages'], 
                                    na.rm = T))
  ...
  benefits_strictly <- dataset %>% dplyr::group_by(emitter, reciever) %>% 
    dplyr::summarise(transfer = sum(weighted_damages2[status_transfer == 'benefits'], 
                                    na.rm = T))
  
  colnames(benefits_strictly)[1] <- "owing"
  colnames(benefits_strictly)[2] <- "owed_to"
  ...
  damages_transfers2_owing <- subset(damages_transfers2_owing, 
                                     owing_real != "Other emitters")
  benefits_transfers2_owing <- subset(benefits_transfers2_owing, 
                                     owing_real != "Others.")
  damages_transfers2_owed_to <- subset(damages_transfers2_owed_to, 
                                      owed_to_real != "Others")
