...
#############################################################################
# read data 
total_damages_k90 <- readRDS(paste0(dropbox_path, "data/output/20230602/total_damages_k90_v2022.rds"))
total_damages_k80 <- readRDS(paste0(dropbox_path, "data/output/20230602/total_damages_k80_v2022.rds"))
total_damages_k90_prod <- readRDS(paste0(dropbox_path, "data/output/20230602/total_damages_k90_prod_v2022.rds"))
total_damages_k90_consump <- readRDS(paste0(dropbox_path, "data/output/20230602/total_damages_k90_consump_v2022.rds"))
...
prep_data_for_sankey <- function(dataset){
  ...
  damages_and_benefits_trasnfers <- rbind(damages_strictly,
                                          benefits_strictly)
  ...
  damages_and_benefits_trasnfers <- left_join(damages_and_benefits_trasnfers,
                                              damages_strictly_sums,
                                              by = c("owed_to"))
  unique(damages_and_benefits_trasnfers$transfer_status)
  colnames(damages_and_benefits_trasnfers)[9] <- "rank_damages_owed_to"
  
  damages_strictly_sums_owe <- damages_strictly_sums_owe %>% dplyr::select(c("owing", "rank"))
  damages_and_benefits_trasnfers <- left_join(damages_and_benefits_trasnfers,
                                              damages_strictly_sums_owe,
                                              by = c("owing"))
  colnames(damages_and_benefits_trasnfers)[10] <- "rank_damages_owing"
  
  benefits_strictly_sums_owe <- benefits_strictly_sums_owe %>% dplyr::select(c("owing", "rank"))
  damages_and_benefits_trasnfers <- left_join(damages_and_benefits_trasnfers,
                                              benefits_strictly_sums_owe,
                                              by = c("owing"))
  colnames(damages_and_benefits_trasnfers)[11] <- "rank_benefits_owing"
  ...
  damages_and_benefits_trasnfers <- left_join(damages_and_benefits_trasnfers,
                                              ...
                                              by = c("owed_to"))
  colnames(damages_and_benefits_trasnfers)[12] <- "rank_benefits_owed_to"
  ...
  damages_and_benefits_trasnfers$transfer <- damages_and_benefits_trasnfers$transfer * (-1) 
  
  damages_back <- subset(damages_and_benefits_trasnfers, transfer_status == "damages") 
  ...
  benefits_back <- subset(damages_and_benefits_trasnfers, transfer_status == "benefits") 
  ...
  damages_and_benefits_trasnfers2 <- rbind(damages_back,
                                           benefits_back)
  
  damages_trasnfers2_owing <- damages_and_benefits_trasnfers2 %>% 
    dplyr::group_by(owing_real) %>% 
    dplyr::summarise(transfer = sum(transfer[transfer > 0], na.rm = T))
  damages_trasnfers2_owing <- damages_trasnfers2_owing[order(-damages_trasnfers2_owing$transfer),] 
  damages_trasnfers2_owing$rank <- 1:nrow(damages_trasnfers2_owing)
  
  benefits_trasnfers2_owing <- damages_and_benefits_trasnfers2 %>% 
    dplyr::group_by(owing_real) %>% 
    dplyr::summarise(transfer = sum(transfer[transfer < 0], na.rm = T))
  benefits_trasnfers2_owing <- benefits_trasnfers2_owing[order(-benefits_trasnfers2_owing$transfer),] 
  benefits_trasnfers2_owing$rank <- 1:nrow(benefits_trasnfers2_owing)
  
  damages_trasnfers2_owed_to <- damages_and_benefits_trasnfers2 %>% 
    dplyr::group_by(owed_to_real) %>% 
    dplyr::summarise(transfer = sum(transfer[transfer > 0], na.rm = T))
  damages_trasnfers2_owed_to <- damages_trasnfers2_owed_to[order(-damages_trasnfers2_owed_to$transfer),] 
  damages_trasnfers2_owed_to$rank <- 1:nrow(damages_trasnfers2_owed_to)
  
  benefits_trasnfers2_owed_to <- damages_and_benefits_trasnfers2 %>% 
    dplyr::group_by(owed_to_real) %>% 
    dplyr::summarise(transfer = sum(transfer[transfer < 0], na.rm = T))
  benefits_trasnfers2_owed_to <- benefits_trasnfers2_owed_to[order(-benefits_trasnfers2_owed_to$transfer),] 
  benefits_trasnfers2_owed_to$rank <- 1:nrow(benefits_trasnfers2_owed_to)
  
  damages_and_benefits_trasnfers2 <- damages_and_benefits_trasnfers2 %>% 
    dplyr::mutate(owing_real = case_when(owing_real == "Other emitters" ~ paste0("Other emitters ($", 
                                                                                 round(damages_trasnfers2_owing$transfer[damages_trasnfers2_owing$owing_real == "Other emitters"]/1000000000000,2), "T)"),
                                         TRUE ~ owing_real))
  
  
  damages_and_benefits_trasnfers2 <- damages_and_benefits_trasnfers2 %>% 
    dplyr::mutate(owing_real = case_when(owing_real == "Others." ~ paste0("Others ($", 
                                                                          round(benefits_trasnfers2_owing$transfer[benefits_trasnfers2_owing$owing_real == "Others."]/-1000000000000,2), "T)"),
                                         TRUE ~ owing_real))
  
  damages_and_benefits_trasnfers2 <- damages_and_benefits_trasnfers2 %>% 
    dplyr::mutate(owed_to_real = case_when(owed_to_real == "Others" ~ paste0("Others ($", 
                                                                             round(damages_trasnfers2_owed_to$transfer[damages_trasnfers2_owed_to$owed_to_real == "Others"]/1000000000000,2), "T)"),
                                           TRUE ~ owed_to_real))
  
  damages_and_benefits_trasnfers2 <- damages_and_benefits_trasnfers2 %>% 
    dplyr::mutate(owed_to_real = case_when(owed_to_real == "Other emitters." ~ paste0("Other emitters ($", 
                                                                                      round(benefits_trasnfers2_owed_to$transfer[benefits_trasnfers2_owed_to$owed_to_real == "Other emitters."]/-1000000000000,2), "T)"),
                                           TRUE ~ owed_to_real))
  
  
  damages_and_benefits_trasnfers2 <- ungroup(damages_and_benefits_trasnfers2)
  damages_and_benefits_trasnfers2 <- damages_and_benefits_trasnfers2 %>% dplyr::select(c("owing_real", 
                                                                                         "owed_to_real",
                                                                                         "transfer"))
  damages_and_benefits_trasnfers2 <- subset(damages_and_benefits_trasnfers2, transfer != 0)
  
  return(damages_and_benefits_trasnfers2)
}
...
# data is ready 
write_rds(damages_and_benefits_k90, paste0(fig_prepped_dta, "damages_and_benefits_k90.rds"))
write_rds(damages_and_benefits_k80, paste0(fig_prepped_dta, "damages_and_benefits_k80.rds"))
write_rds(damages_and_benefits_k90_prod, paste0(fig_prepped_dta, "damages_and_benefits_k90_prod.rds"))
write_rds(damages_and_benefits_k90_consump, paste0(fig_prepped_dta, "damages_and_benefits_k90_consump.rds"))
