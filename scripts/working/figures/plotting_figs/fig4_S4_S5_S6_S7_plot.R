...
damages_and_benefits_k60 <- readRDS(paste0(fig_prepped_dta,run_date, "/damages_and_benefits_k60.rds"))
...
# now let us plot 
ggplot(damages_and_benefits_k90, aes(y = transfer, stratum = stratum, alluvium = flow, x = x, label = stratum)) +
  theme_minimal() +
  geom_stratum() +
  geom_hline(yintercept = 0) +
  labs(y = "Benefits from warming ($USD) and Damages from warming ($USD)", x = "") +
  scale_x_discrete(limits = c("owing_real", "owed_to_real"), labels = c("owing_real" = "Emitter", "owed_to_real" = "Recipient"))
...
head(damages_and_benefits_k90)
summary(damages_and_benefits_k90)
################################################################################ Sankey-> k = 1980
# we need to use "owed to" as a category for determining the different colors
...
################################################################################ Sankey-> k = 1960
# we need to use "owed to" as a category for determining the different colors
owed_to_colors <- subset(damages_and_benefits_k60, x == "owed_to_real")
owed_to_colors <- owed_to_colors %>% dplyr::select(c("flow", "stratum"))
# rename before joining
colnames(owed_to_colors)[2] <- "owed_to_real"
# join data 
damages_and_benefits_k60 <- left_join(damages_and_benefits_k60,
                                      owed_to_colors,
                                      by = c("flow"))
# determine the year of k 
damages_and_benefits_k60$year_k <- 1960

# now this is a manual step. But we need to determine which labels we need to 
# take outside the boxes and send a line to 
label_out_rect_k60 <- c(1:7, 9:18)
label_out_rect_right_k60 <- c(1:31, 39:46)

# now let us plot 
sankey_k60 <- ggplot(damages_and_benefits_k60,
                     aes(y = transfer, 
                         stratum = stratum, 
                         alluvium = flow, 
                         x=x, 
  ...
  scale_x_discrete(limits = c("owing_real", "owed_to_real"),
                   labels=c("owing_real"="Emitter", "owed_to_real"="Recipient")) + 
  geom_text(stat="stratum", 
            aes(label=ifelse(after_stat(deposit)%not_in%label_out_rect_k60 &after_stat(deposit)%in%label_out_rect_right_k60,
                             as.character(after_stat(stratum)),"")),
            size=9) +  
  ggrepel::geom_text_repel(
    stat="stratum", 
    aes(label=ifelse(after_stat(deposit)%in%label_out_rect_k60,
                     as.character(after_stat(stratum)),"")),
    nudge_x = -0.5,
    direction="y",
    hjust=-0.06,
  ...
  ggrepel::geom_text_repel(
    stat="stratum", 
    aes(label=ifelse(after_stat(deposit)%not_in%label_out_rect_right_k60,
                     as.character(after_stat(stratum)),"")),
    nudge_x = 0.5,
    direction="y",
    hjust=-0.5,
    ...
    size=9
                        ...
                        breaks = unique(damages_and_benefits_k60$owed_to_real)) +
  scale_y_continuous(labels = addUnits, limits = range(-250000000000,325000000000000)) 
...
save_plot(paste0('figures/', run_date, '/figS4.pdf'),plot=sankey_k80, 
          base_width=26.5, base_height=28, scale=1.1)
save_plot(paste0('figures/', run_date, '/figS7.pdf'),plot=sankey_k90_prod, 
          base_width=26.5, base_height=28, scale=1.1)
save_plot(paste0('figures/', run_date, '/figS6.pdf'),plot=sankey_k90_consump, 
          base_width=26.5, base_height=28, scale=1.1)
save_plot(paste0('figures/', run_date, '/figS5.pdf'),plot=sankey_k60, 
          base_width=26.5, base_height=28, scale=1.1)
